          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                {/* Avatar */}
                <td style={{ ...td, width: 48, paddingRight: 0 }} data-label="">
                ...
                <td style={td} data-label="Nombre"><strong>{u.full_name}</strong></td>
                <td style={{ ...td, color: "#6b7280" }} data-label="Email">{u.email}</td>
                <td style={td} data-label="Rol">
                  <span style={roleBadge(u.role)}>
                    {u.role === "superadmin" ? "Superadmin" : "Investor"}
                  </span>
                </td>
                <td style={{ ...td, color: "#9ca3af", fontSize: "0.8rem" }} data-label="Registrado">
                ...
                {isSuperAdmin && (
                  <td style={td} data-label="">
                    <ActionsMenu actions={[{ label: "Editar", href: `/${lang}/admin/users/${u.id}/edit` }]}>
                      <DeleteUserButton menuItem userId={u.id} currentUserId={Number(session?.sub)} />
                    </ActionsMenu>
                  </td>
                )}
              </tr>
