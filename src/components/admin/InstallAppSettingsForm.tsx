...
export default function InstallAppSettingsForm({ initialEnabled }: Props) {
  ...
  async function toggle() {
    ...
    const res = await fetch("/api/admin/settings/install-app", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ install_app_enabled: next }),
    });
  ...
  return (
    <div style={wrap}>
      <h2 style={title}>Instalar app</h2>
      <p style={hint}>
        Mientras esté habilitado, el sitio ofrece instalar la app (banner flotante y accesos en
        los menús). Deshabilitarlo lo oculta por completo en todo el sitio público.
      </p>
